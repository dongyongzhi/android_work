...
import com.yfcomm.launcher.DropTarget.DragObject;
import com.yfcomm.launcher.LauncherSettings.LauncherInfo;
...
public final class Launcher extends Activity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mModel = app.setLauncher(this);//全局处理结构对象 
        
        mIconCache = app.getIconCache();//快捷图标的对象
      
        mDragController = new DragController(this); //拖动管理的类
        
        mInflater = getLayoutInflater();//获取当前view容器

        mAppWidgetManager = AppWidgetManager.getInstance(this);//小部件管理Sevice
        ...
        if (!mModel.isAllAppsLoaded()) { //判断进程是否加载APP完成，如果没完成就 显示进度条。
    ...
    protected void onResume() {
        ...
        if (!LauncherApplication.DISPLAY_WALLPAPER_IN_ALLAPP) {
    ...
    */
    ...
    private void setupViews() {
     ...
     //   mQsbDivider = findViewById(R.id.qsb_divider);
        ...
        // Setup the workspace
        mWorkspace.setHapticFeedbackEnabled(false);//触摸时，没触感反馈
        
        mWorkspace.setOnLongClickListener(this); // 长按--效果
        ...
        if (LauncherApplication.DISPLAY_WALLPAPER_IN_ALLAPP) {
            mAppsCustomizeTabHost.getBackground().setAlpha(0);
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (LauncherApplication.LUNCHER_SORT_ENABLED) {
...
//        }
    ...
    public boolean onLongClick(View v) {
        ...
        if (!isDraggingEnabled()) return false;           //是否不可绘制   
        if (isWorkspaceLocked()) return false;            //桌面 锁定
        if (mState != State.WORKSPACE) return false;      //状态是否为 WORKSPACE  

        if (!(v instanceof CellLayout)) {
            v = (View) v.getParent().getParent();  //获取顶层CellLayout
        }

        resetAddInfo();//重置新增信息 
        
        CellLayout.CellInfo longClickCellInfo = (CellLayout.CellInfo) v.getTag(); //是否在有效对话框内
    ...
    void setBehindWindowDim(boolean dimon) {
        if (dimon) {
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);
            getWindow().setDimAmount(LauncherApplication.WALLPAPER_DIM_IN_ALLAPP); // ref: 0.2f
        } else {
    ...
    private void showAppsCustomizeHelper(final boolean animated, final boolean springLoaded) {
        if (mStateAnimation != null) { //负责动画的对象
            mStateAnimation.setDuration(0);//动画时长
        ...
        if (animated) {
            ...
            mStateAnimation.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    if (!LauncherApplication.DISPLAY_WALLPAPER_IN_ALLAPP && !animationCancelled) {
                        updateWallpaperVisibility(false);
                    }
    ...
    void showAllApps(boolean animated) {
        ...
        // Pause the auto-  of widgets until we are out of AllApps
        mUserPresent = false;
        updateRunning();
        closeFolder();
        ...
        Runnable setAllAppsRunnable = new Runnable() {
            public void run() {
                if (mAppsCustomizeContent != null) {
                    ...
                    int sort_by = mSharedPreferences.getInt(SORT_BY_METHOD,
                            Utilities.SORT_BY_ALPHABET);
    ...
    private class MyPreferenceChangeListener implements
        ...
        public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
            if(key.equals(SORT_BY_METHOD)){
                int sortBy = sharedPreferences.getInt(SORT_BY_METHOD, Utilities.SORT_BY_ALPHABET);

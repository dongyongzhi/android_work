...
import com.hftcom.ElecException;
...
public class HttpClientUtil {
	...
	public static JSONObject sendSSLPostRequest(String postUrl,Map<String,Object> params) throws ElecException {
		...
		try {
			...
			throw new ElecException(POSP_RESPONSE_500);  //posp中心返回的数据有问题
		} catch (NoSuchAlgorithmException e) {
			Log.e(TAG, e.getMessage(),e);
			throw new ElecException(POSP_CLIENT_FAIL);
		/*} catch (NoSuchProviderException e) {
			...
			Log.e(TAG, e.getMessage(),e);
			throw new ElecException(POSP_CONNECTION_FAIL); //连接失败，己断网、网络无效、posp服务器连接不上？
		} finally {
			if(os!=null)
				try {
					os.close();
	...
	public static String sendPostRequest(String postUrl,Map<String,String> params,String formName,String fileName,byte[] jpg){
		...
		try {
			...
			ElecLog.e(HttpClientUtil.class, e.getMessage());
			 //连接失败，己断网、网络无效、posp服务器连接不上？
			throw new ElecException(POSP_CONNECTION_FAIL);

...
import com.hftcom.R;
import com.hftcom.widget.ElecDialog;
import com.hftcom.widget.ElecDialog.Style;
...
public class MessageBox {
	 ...
	 * 自定义对话框
	 */
	public static void showConfirmBox(Context ctx,CharSequence cs, CharSequence okStr,CharSequence cancelStr,DialogInterface.OnClickListener ok,DialogInterface.OnClickListener cancel){
		try{
			new ElecDialog.Builder(ctx,Style.CONFIRM).setTitle(R.string.dialog_title_warn)
			.setMessage(cs).setOkButton(okStr).setCancelButton(cancelStr)
			.setConfirmButtonListener(ok)
			.setCancelButtonListener(cancel).show();
		}catch(Exception e){
			ElecLog.w(MessageBox.class, e.getMessage());
		}
	}

rem
rem $Header: fgacdemo.sql 08-mar-2000.11:26:53 dmwong Exp $
rem
rem Copyright (c) 1999,, 2000 Oracle Corporation.  All rights reserved.
rem
rem   NAME
...
DROP TRIGGER TRIGGER SECDEMO.SET_EXPENSE_CTX ;
DROP USER MOREAU;
DROP USER MILLER;
...
EXECUTE DBMS_RLS.DROP_POLICY('secdemo','EXP_REPORT_SUM_VIEW', \
  'exp_report_VIEW_policy');
EXECUTE DBMS_RLS.ADD_POLICY('secdemo','EXP_REPORT_SUM_VIEW', \
  'exp_report_VIEW_policy','secdemo','exp_security.empview_sec','SELECT');

rem POLICY #2
rem     YOU CAN DELETE YOUR REPORTS ONLY
EXECUTE DBMS_RLS.DROP_POLICY('secdemo','exp_report','exp_report_policy');
EXECUTE DBMS_RLS.ADD_POLICY('secdemo','exp_report','exp_report_policy', \
  'secdemo','exp_security.empnum_sec','DELETE');

rem POLICY #2
rem     YOU CAN INSET A REPORT FOR YOURSELF ONLY
EXECUTE DBMS_RLS.DROP_POLICY('secdemo','exp_report', \
  'exp_report_insert_policy');
EXECUTE DBMS_RLS.ADD_POLICY('secdemo','exp_report','exp_report_insert_policy', \
  'secdemo','exp_security.empnum_sec','INSERT');
...
EXECUTE DBMS_RLS.ADD_POLICY('secdemo','exp_line','exp_line_policy','secdemo', \
  'exp_security.empnumline_sec','DELETE');
...
EXECUTE DBMS_RLS.ADD_POLICY('secdemo','exp_report', \
  'exp_report_Approve_policy','secdemo','exp_security.ccid_mgr_sec','UPDATE');

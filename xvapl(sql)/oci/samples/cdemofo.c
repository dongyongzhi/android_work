#ifdef RCSID
static char *RCSid =
   "$Header: cdemofo.c 13-sep-2000.21:32:34 emendez Exp $ ";
#endif /* RCSID */
...
char *argv[];
{
  ...
  if (register_callback(srvhp, errhp))
    goto terminate;
...
sb4 register_callback(srvhp, errhp)
...
OCIError *errhp;                                         /* the error handle */
{
  ...
  if (!(failover.fo_ctx =
                       (dvoid *)malloc(strlen("Application Failove Context"))))
  {
  ...
  strcpy((char *)failover.fo_ctx, "Application Failover Context");

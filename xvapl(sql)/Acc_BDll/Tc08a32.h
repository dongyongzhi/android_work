...
#define	MAX_CHANNEL_NO	16 * MAX_CARD_NO
...
#define S_SENDEND		0x16   //add by yezm, 2009.09.01, for mantis 1432
#define S_RINGBACK		0x17   //add by yezm, 2009.09.01, for mantis 1432

#define S_DIALSIG       0x30


#define PACK_64KBPS     0	//support for A-LAW PCM
#define PACK_32KBPS     1	//support for ADPCM
#define PACK_16KBPS     2	//no use
#define PACK_8KBPS      3	//no use
#define PACK_8KBPS_G729 4	//support for g729
...
#define SIG_DOUBLE_RINGBACK		20	//add by jzq for double ringback signal in 08.03.31
...
typedef struct {
	...
	WORD wMajorVer;
	WORD wMinorVer;
	WORD wChType[MAX_CHANNEL_NO_256];
    //add by lanjun fro new record
	WORD wIsNewRecord[MAX_CHANNEL_NO_256];
...
BOOL WINAPI CheckRingEnd(WORD wChnlNo);	//add by jzq in 08.05.07
...
int  WINAPI GetPlayFileOffset ( WORD wChnlNo );
...
typedef struct {
	...
	WORD wChnlNum;
	BYTE cbChType[MAX_CHANNEL_NO_256];
	BYTE cbChnlCardNo[MAX_CHANNEL_NO_256];
	BYTE cbChnlInternal[MAX_CHANNEL_NO_256];
	BYTE cbConnectChnl[MAX_CHANNEL_NO_256];
	BYTE cbConnectStream[MAX_CHANNEL_NO_256];
	BYTE cbDtmfModeVal[MAX_CHANNEL_NO_256];
	BYTE cbIsSupportCallerID[MAX_CHANNEL_NO_256];
} TC_INI_TYPE_MORE;
...
//add by jzq for new_record in 08.03.31
int WINAPI D_AdjustVocVol_D080A_R ( WORD wChnl, char cMode, char cVolAdjust, BOOL bIsD080A_R );
int WINAPI D_AdjustVocVol_ForVB_D080A_R ( WORD wChnl, BYTE byMode, BYTE byVolAdjust, BOOL bIsD080A_R);
//end
...
int	WINAPI	VR_GetRecordWaveData(WORD wChnlNo, char *ppcmBuffer, char* pwaveBuf);
...
//add by lanjun for new record 2007-5-28
BOOL WINAPI DRec_LineDetect (WORD wChnlNo);
//add by lanjun for new record 2007-5-28
...
//add by jzq for double ringback signal on 2008.03.31
int WINAPI SetGenerateSigParam_fordoubleringback(int nSigType, int nFreq1, int nFreq2, double dbAmp1, double dbAmp2, 
							   int nOnTime1, int nOffTime1, int nOnTime2, int nOffTime2, int iSampleRate);
...
// add netxt function in 2006.12.07 for HYT MSK Filter
long WINAPI D_Prepare_MSK_Filter ( WORD wChnlNo, WORD wHandle );
long WINAPI D_Cancel_MSK_Filter ( WORD wChnlNo );
//
...
// -------------------------------------------------------------------------
// -------------------------------------------------------------------------
void WINAPI StartCheckSingleFreqTone(WORD wChnlNo, DWORD dwFreq);
int WINAPI SingleFreqToneCheckResult(WORD wChnlNo);
// -------------------------------------------------------------------------
// -------------------------------------------------------------------------


long WINAPI	ConnectForDoubleChnlRecord( WORD wUserChnlNo, WORD wTrunkChnlTS );
WORD WINAPI DisconnectForDoubleChnlRecord(WORD wUserChnlNo );

//2008.03.10
DWORD WINAPI ReadPromLastFourByte( WORD CardNo );
//end

//add by jzq for DJLM_NEW
//--------------------------------------------------------------------------
//no use except "DATA_FORMAT_ALAW" now!

#define DATA_FORMAT_DIALOGIC_ADPCM		        0x1		// OKI ADPCM 
#define DATA_FORMAT_ALAW				        0x3		// alaw PCM  
#define DATA_FORMAT_PCM					        0x8		// line PCM       
#define DATA_FORMAT_G729				        0x0C	// CELP coder 

int WINAPI DJLM_D160A_Init_New(void);
void WINAPI DJLM_D160A_SetMonitorChnl(WORD wChnl, BOOL bMon);
//end


//2009.03.12 为了获取大于128路的通道信息，增加了以下结构和函数
typedef struct {
	WORD wMemAddr;
	...
	BYTE cbDtmfModeVal[MAX_CHANNEL_NO_256];
	BYTE cbIsSupportCallerID[MAX_CHANNEL_NO_256];
} TC_INI_TYPE_MORE_256;

WORD WINAPI D_GetSysInfoMore_256( TC_INI_TYPE_MORE_256 *pTmpMore);
//2009.03.12 by yezm


int WINAPI StartPlayMem(WORD wChnlNo);
int WINAPI SendToBuff(WORD wChnlNo, char *PlayBuf, DWORD dwPlayLen);
//nMode:0-从麦克风采集的数据,1-从语音文件读出的数据
int WINAPI SendToBuffMuti(WORD wChnlNo, char *PlayBuf, DWORD dwPlayLen, int nMode);


//2007, add by Jiangzq
BOOL WINAPI RingDetect_Ex(WORD wChnlNo);
//end

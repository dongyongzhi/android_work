...
const byte perm_choice1_C[28] = {
	56, 48, 40, 32, 24, 16,  8,
	 0, 57, 49, 41, 33, 25, 17,
	 9,  1, 58, 50, 42, 34, 26,
	18, 10,  2, 59, 51, 43, 35};
	
const byte perm_choice1_D[28] = {
	62, 54, 46, 38, 30, 22, 14,
	 6, 61, 53, 45, 37, 29, 21,
	13,  5, 60, 52, 44, 36, 28,
	20, 12,  4, 27, 19, 11,  3};

const byte perm_choice2[48] = {
	13, 16, 10, 23,  0,  4,
	 2, 27, 14,  5, 20,  9,
	22, 18, 11,  3, 25,  7,
	15,  6, 26, 19, 12,  1,
...
const byte shifts_table[16] = {
	1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};

const byte sel_table[512] = {
	/* s1 */
...
void do_ip(byte *input, byte *output, const byte *table){
...
void rotate(byte* ptr, byte times){
 ...
 ntimes = 8 - times;
 bit = (ptr[0] >> (ntimes-4)) & 0xf0; /* save LS Bit of lower word before shifting */
 ptr[0] <<= times;
 ptr[0] |= (ptr[1] >> ntimes);
 ptr[1] <<= times;
 ptr[1] |= (ptr[2] >> ntimes);
...
void _DESByByte(BYTE *dg,BYTE *key_ptr,byte *output)
{
	//BYTE des_buf[8];
...
void DESByByte(BYTE *dg,BYTE *key_ptr,byte *output)
{
 //   BYTE des_buf[8];
    memcpy(key,key_ptr,8);
    gen_keys();
...
void DES_Encryption (byte *deskey, byte *input, byte *output)
{
	byte i;
	for (i = 0;i< 8; i++)
    	key[i] = deskey[i];
    gen_keys();
    cipher(input, output, 0);
}    

void DES_Decryption (byte *deskey, byte *input, byte *output)
{
	byte i;
	for (i = 0;i< 8; i++)
    	key[i] = deskey[i];

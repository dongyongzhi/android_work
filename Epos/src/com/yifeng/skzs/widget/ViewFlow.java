 ...
 */
package com.yifeng.skzs.widget;
...
public class ViewFlow extends AdapterView<Adapter> {
	...
	public boolean isSendHandler=false;
	...
	private long timeSpan = 3000;//延时执行
  ...
  //通过延迟消息实现自动播放，使用时通过调用该方法来启动自动播放功能
	public void startAutoFlowTimer(){
		handler = new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				int nextPage = add ? ++mCurrentScreen : --mCurrentScreen;
	...
	private void postViewSwitched(int direction) {
	    ...
	    if(mCurrentBufferIndex>mLoadedViews.size()-1)
	    	return;
	...
	private void logBuffer() {
		...
		if(this.isSendHandler){
	        ...
	        // 将进度放入Message中 
		    data.putInt("currentIndex",mCurrentAdapterIndex);
		    msg.setData(data);
	        sendHandler.sendMessage(msg);
        }
	}
	
	public void setSendHandler(boolean isSendHandler) {

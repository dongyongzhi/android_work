	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data){  
		...
		case RESULT_OK:
			if(currentPosition!=-1){
				...
				//获取返回交易类型
				String messageType = data.getStringExtra(EXTRA_TRANS_RESULT);
				 
				if(MessageType.CANCEL_PURCHASE.equals(messageType)) {
					source.setStatus(OrderState.REPEAL);
					orderAdapter.notifyDataSetChanged(); //数据变更
				} else if (MessageType.RETURNS.equals(messageType)) {

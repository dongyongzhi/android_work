...
public class TransResponse  extends ElecResponse implements Parcelable {
	...
	public static final Parcelable.Creator<TransResponse> CREATOR = new Parcelable.Creator<TransResponse>() {
		...
		public TransResponse createFromParcel(Parcel source) {
			...
			item.messageType = source.readString();
			...
			item.business =source.readString() ;
			item.businessNo =source.readString() ;
			item.termNo =source.readString() ;
	...
	public void setErrCode(String errCode) {
		...
		this.setErrMsg(ISO8583Util.getTransMessage(errCode));
	}
	...
	public String getMessageTypeName(){
		if(messageType == null)
			return "";
		else if(messageType.equals(MessageType.PRE_AUTHORIZE))
			return "预授权";
		else if(messageType .equals( MessageType.ADD_PRE_AUTHORIZE))
			return "追加预授权";
		else if(messageType.equals(MessageType.PURCHASE))
			return "消费";
		else if(messageType.equals(MessageType.QUERYBALANCE))
			return "查询";
		else if(messageType.equals(MessageType.RETURNS))
			return "退货";
		else if(messageType.equals(MessageType.CANCEL_PURCHASE))
			return "消费撤销";
		else if(messageType.equals(MessageType.CANCEL_PRE_AUTHORIZE))
			return "预授权撤销";
		else
			return "";
	}
	...
	public static Parcelable.Creator<TransResponse> getCreator() {
		return CREATOR;
	}

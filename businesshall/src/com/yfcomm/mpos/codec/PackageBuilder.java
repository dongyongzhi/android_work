...
public class PackageBuilder {
	...
	/** 上报输密结果 **/
	public static final int CMD_UPLOAD_KEYBORD = 0x0403;
	...
	/* 消费触发 */
	public static final int CMD_XFCF = 0x0801;
	/* 消费触发应答 */
	public static final int CMD_XFYD = 0x0802;

	/* 刷卡请求 */
	public static final int CMD_SKQQ = 0x0803;
	/* 刷卡应答 */
	public static final int CMD_SKYD = 0x0804;

	/* 身份证数据下发 */
	public static final int CMD_SFXF = 0x0901;
	/* 身份证数据上传 */
	public static final int CMD_SFSC = 0x0902;
	...
	public static DevicePackage syn(int cmd, byte[] body) {
		DevicePackage pack = new DevicePackage(PackageType.SYN.getType(), getNextPackageSequence(), cmd, body);
		return pack;
	}
	...
	public static DevicePackage ackError(byte sequence, int cmd, int errorCode) {
		return new DevicePackage(PackageType.ACK_ERROR.getType(), sequence, cmd, new byte[] { (byte) errorCode });
	}
}

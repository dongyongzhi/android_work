 ...
 * 写卡
 */
public class WriteSLECardTask extends AsyncTask<Object, Object, Integer> {

	private final static YFLog logger = YFLog.getLog(WriteSLECardTask.class);
	private final SwiperListener listener;
	private final DeviceComm deviceComm;
	private byte[] result;

	public WriteSLECardTask(DeviceComm deviceComm, SwiperListener listener) {
	...
	protected Integer doInBackground(Object... params) {
		try {
			...
			if(recvData[0] == 0x00){
				...
				System.arraycopy(data, 0, body, 5, data.length);
				recv = this.deviceComm.execute(PackageBuilder.syn(PackageBuilder.CMD_SLE_WRITE, body));
				result = recv.getBody();

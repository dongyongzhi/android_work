...
import com.yifengcom.yfpos.tlv.support.BERTLV;
...
public class SwiperTask extends AsyncTask<Void,Object,Integer>{
	...
	protected Integer doTask() throws Exception {
		...
		}else if(recvData[0] == 0x07){
			code = this.readRFID(ack);
			if(code !=ErrorCode.SUCC.getCode()) {
				return code;
			}
		}
	 ...
	 * @throws InterruptedException 
	 */
	private int readRFID(DevicePackage ack) throws InterruptedException {
		setIc(true);
		...
		logger.d("获取Q联机流程结果");
		Thread.sleep(200);
		ack = this.deviceComm.execute(PackageBuilder.syn(PackageBuilder.CMD_Q_ONLINE_READ));
		byte[] ic = ack.getBody();

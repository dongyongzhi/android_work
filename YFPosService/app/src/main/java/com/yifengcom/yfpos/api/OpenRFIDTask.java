 ...
 *
 */
public class OpenRFIDTask extends AsyncTask<Void, Object, Integer> {

	private final static YFLog logger = YFLog.getLog(OpenRFIDTask.class);
	private final SwiperListener listener;
	private final DeviceComm deviceComm;
	private byte[] result;

	public OpenRFIDTask(DeviceComm deviceComm, SwiperListener listener) {
	...
	protected Integer doInBackground(Void... params) {
		try {
			...
			logger.d("请求打开射频");
			this.deviceComm.execute(PackageBuilder.syn(PackageBuilder.CMD_RFID_OPEN, timeout));
			logger.d("等待寻卡结果上报");
			recv = this.deviceComm.recv(new DevicePackageSequence(PackageBuilder.CMD_RFID_FIND_RESULT, PackageBuilder.getNextPackageSequence()), 60 * 1000);
			if (recv == null) {

...
public class IconCache {
    ...
    public void getTitleAndIcon(ApplicationInfo application, LauncherActivityInfo info,
        ...
        synchronized (mCache) {
            CacheEntry entry = cacheLocked(application.componentName, info, labelCache,
                    info.getUser(), application.unreadNum);
    ...
    public Bitmap getIcon(Intent intent, UserHandle user) {
        synchronized (mCache) {
            ...
            CacheEntry entry = cacheLocked(component, launcherActInfo, null, user, -1);
            return entry.icon;
        }
    }
    ...
    public Bitmap getIcon(ComponentName component, LauncherActivityInfo info,
        ...
        synchronized (mCache) {
            ...
            CacheEntry entry = cacheLocked(component, info, labelCache, info.getUser(), -1);
            return entry.icon;
        }
    }
    ...
    private CacheEntry cacheLocked(ComponentName componentName, LauncherActivityInfo info,
            HashMap<Object, CharSequence> labelCache, UserHandle user, int unreadNum) {
        CacheKey cacheKey = new CacheKey(componentName, user);
        CacheEntry entry = mCache.get(cacheKey);
        boolean condition = (mContext.getResources().getBoolean(
                R.bool.config_telcel_homescreen_enabled) || mContext
                .getResources().getBoolean(R.bool.config_claro_homescreen_enabled))
                && info.getComponentName().getPackageName().toString().equalsIgnoreCase(
                STK_PACKAGE_NAME);

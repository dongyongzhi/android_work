...
class AllAppsList {
    ...
    public ApplicationInfo unreadNumbersChanged(Context context, ComponentName component,
        ...
        LauncherApps launcherApps = (LauncherApps)
                context.getSystemService(Context.LAUNCHER_APPS_SERVICE);
        final List<LauncherActivityInfo> matches = launcherApps.getActivityList(
                component.getPackageName(), android.os.Process.myUserHandle());

        for (LauncherActivityInfo launcherActivityInfo : matches) {
            if (component.getPackageName().equals(
                ...
                if (applicationInfo == null) {
                    ...
                    mIconCache.remove(applicationInfo.componentName);
                    mIconCache.getTitleAndIcon(applicationInfo, launcherActivityInfo, null);

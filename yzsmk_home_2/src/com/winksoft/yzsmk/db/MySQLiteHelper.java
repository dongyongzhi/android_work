...
public class MySQLiteHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		String sql = "create table if not exists " + public_define.TBNAME + "("
				+ "id integer primary key," + public_define.packetname
				+ " varchar," + public_define.title + " varchar,"
				+ public_define.curpageid + " integer,"
				+ public_define.curItemindex + " integer,"
				+ public_define.isnull + " integer," + public_define.icon
				+ " BINARY)";
		db.execSQL(sql);
	}
	...
	public void InsertAppData(AppInfo nativeapp) {
		...
		values.put(public_define.curpageid, nativeapp.curpageid);
		values.put(public_define.curItemindex, nativeapp.curindex);
		values.put(public_define.isnull, nativeapp.IsNull);

		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		((BitmapDrawable) nativeapp.icon).getBitmap().compress(
	...
	public List<AppInfo> GetlistAppData() {
		...
		Cursor cursor = db.query(public_define.TBNAME, null, null, null, null,
				null, "id asc");

		for (cursor.moveToFirst(); !(cursor.isAfterLast()); cursor.moveToNext()) {

			AppInfo napp = new AppInfo();
			napp.title = cursor.getString(cursor
					.getColumnIndex(public_define.title));
			napp.Packetname = cursor.getString(cursor
					.getColumnIndex(public_define.packetname));
			napp.curpageid = cursor.getInt(cursor
					.getColumnIndex(public_define.curpageid));
			napp.curindex = cursor.getInt(cursor
					.getColumnIndex(public_define.curItemindex));
			napp.IsNull = cursor.getInt(cursor
					.getColumnIndex(public_define.isnull));
	...
	public void deleteOneAppinfo(String Condtion) {
		...
		SQLiteDatabase db = getWritableDatabase();
		db.delete(public_define.TBNAME, Condtion, null);
		db.close();
	}

	public void swappostion(AppInfo srcApp, AppInfo desApp) {

		UpdateAppData(srcApp, "id=?",
				new String[] { Integer.toString(srcApp.id) });
		UpdateAppData(desApp, "id=?",
				new String[] { Integer.toString(desApp.id) });
	}
